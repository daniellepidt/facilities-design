...
def event_generator(aisle: Aisle, curr_time: float, request: Counter, shuttle_c_max: dict) -> None:
    """
    Generates events
    """
    ...
    while True:
        ...
        if np.any(relevant_locations == 1.0):
            ...
            for s in aisle.shuttles:
                ...
                relevant_times[:][s.floor] += shuttle_c_max[s.floor] 
            ...
            shuttle_c_max[i[0]] = time_until_shuttle_and_elevator_meet + shuttle_unload_time
            log(
                curr_time,
...
if __name__ == "__main__":
    ...
    for request in REQUESTS:
        ...
        events_list = []
        shuttle_c_max = {shuttle : 0 for shuttle in range(0,8)}
        print("Starting to handle a new request")
        print(request)
        # Creating First events
        event_generator(aisle, curr_time, request, shuttle_c_max)
